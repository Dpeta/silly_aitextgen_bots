...
rust_ai = aitextgen(model_folder=os.path.join("rustye2", "rustmodel"),
                    tokenizer_file=os.path.join("rustye2",
                                                "rustmodel",
                                                "aitextgen.tokenizer.json"))
rust_quote = ("fiwst thing i noticed abouwt u: "
              "'now thewe's a wady who couwwd wesowve any buwtton-based dispuwtes,' i thouwght. ")
rust_activity = discord.Game(rust_quote)
rust = discord.Client(intents=intents,
    ...
    async def message(self, message):
        if message.author == self.client.user:
            ...
            while not output.strip():
                print(t)
                output = rust_ai.generate_one(temperature=t,
                                              prompt='\n\n' + channel_msg_str,
