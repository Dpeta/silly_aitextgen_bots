...
harry_ai = aitextgen(
    model_folder=os.path.join("harrier2"),
    tokenizer_file=os.path.join("harrier2", "aitextgen.tokenizer.json"),
)
harry = discord.Client(
    intents=intents,
...
class HarryBot:
    ...
    async def message(self, message):
        if message.author == self.client.user:
            ...
            channel_msg_str = (
                message.content.replace("!h ", "").replace("!h", "") + ".\n"
            )
