...
jasprose_ai = aitextgen(model_folder=os.path.join("jasprosePro",
                                                  "model3"),
                        tokenizer_file=os.path.join("jasprosePro",
                                                    "model3",
                                                    "aitextgen.tokenizer.json"))
jasprose_activity = discord.Activity(name="!j", details="!j", type=discord.ActivityType.playing)
jasprose = discord.Client(intents=intents,
    ...
    async def message(self, message):
        if message.author == self.client.user:
            ...
            try:
                ...
                await message.channel.send("temp=%s" % self.temp,
                                       allowed_mentions=allowedMentions)
            ...
            while not await is_valid(output):
                print(t)
                output = jasprose_ai.generate_one(temperature=t,
